...
import { Cliente, Vehiculo, Servicio, Repuesto, OrdenFormData, OrdenItem } from '../types';

interface OrdenFormProps {
  orden?: any;
  onSave: (orden: any) => void;
  onCancel: () => void;
}

export default function OrdenForm({ orden, onSave, onCancel }: OrdenFormProps) {
  const [formData, setFormData] = useState<OrdenFormData>({
    cliente: null,
    vehiculo: null,
    descripcion: '',
    observaciones: '',
    kilometrajeEntrada: undefined,
  ...
  useEffect(() => {
    ...
    if (orden) {
      setFormData({
        ...
        kilometrajeSalida: orden.kilometrajeSalida,
        items: orden.items || []
      });
    } else {
      // Generar número de orden automático
      const numero = `ORD-${new Date().getFullYear()}${String(new Date().getMonth() + 1).padStart(2, '0')}${String(new Date().getDate()).padStart(2, '0')}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`;
  ...
  const handleItemChange = (index: number, field: keyof OrdenItem, value: any) => {
    setFormData(prev => {
  ...
  const handleSave = () => {
    ...
    const ordenData = {
      numero: orden?.numero || `ORD-${Date.now()}`,
      ...
      total: calcularTotal(),
      kilometrajeEntrada: formData.kilometrajeEntrada,
      kilometrajeSalida: formData.kilometrajeSalida
    };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            {formData.vehiculo && (
              <div className="bg-gray-700 p-3 rounded">
                ...
                {formData.vehiculo.kilometraje && (
                  <div className="text-gray-300 text-sm">Kilometraje: {formData.vehiculo.kilometraje.toLocaleString()} km</div>
                )}
              </div>
            )}
          </div>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Kilometraje de Entrada</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Kilometraje de Salida</label>
            <input
              type="number"
